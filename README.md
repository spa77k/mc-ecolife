...
**ログインボーナス（カレンダー式）**と**友達招待報酬**を提供します。
...
- 任意依存: Vault＋経済プラグイン（友達招待の通貨報酬に使用。未導入でもログインボーナスは動作）

## 友達招待

新規が `/invite <名前>` で紹介者を登録し、累計2時間遊ぶと紹介者2000S・新規1000Sを受け取れます。`/invite` で招待実績のGUI、`/invite top` でランキングを表示します。

同一IPの拒否、家族の手動救済、SQLite保存、支払い障害の復旧を含む詳細は [友達招待の運営ガイド](docs/invite.md) を参照してください。
...
カレンダーのマスとは別に、「これまでの記録」として2つの数字を出します。

```text
9月のカレンダー: 5 / 30 マス          ← 今月の報酬がどこまで進んだか
これまでの記録: 連続 15日 / 累計 15日   ← これまで何日入ったか（報酬とは無関係）
```

**マスは月が変わると1マス目に戻りますが、連続ログインは月をまたいでも続きます。**
そのため「連続35日なのに2マス目」という状態は正常です。役割が違う数字なので、行を分けています。

- **連続ログイン日数**: 前日から途切れずに続いている日数。1日でも空くと1に戻ります
- **累計ログイン日数**: これまでに受け取った日数の通算
- **最長連続日数**: いまの連続が最長に届いていないときだけ、`最長は 15日でした。` と出ます

区切りは受け取り判定と同じ日本時間の朝4時です（`config.yml` の `day.reset-hour`）。

数えているのは「ログインボーナスを受け取った日」です。`ecolife.daily` 権限がない人や、
`login-bonus.enabled` を `false` にしていた期間は数に入りません。

連続日数に応じた追加報酬はありません。報酬はあくまでカレンダーのマスだけで決まります。
...
| `/ecolife info <名前>` | `ecolife.admin` | OP | 今月のマス数・最終受け取り日・通算・皆勤月数・連続日数・最長連続日数を見る |
...
「最終受け取り日」「通算回数」「皆勤した月数」「連続日数」「最長連続日数」「連続の起点」を
保存します。受け取りのたびに非同期で書き込み、サーバー停止時にも書き切ります。

連続日数の判定には `last-claim`（最終受け取り日）ではなく `streak-day`（連続を最後に伸ばした日）を
使います。この2つを分けているのは、過去のログイン記録から遡って連続日数を入れたとき、
`last-claim` を起点にすると初回の受け取りで連続が1に潰れてしまうためです。

`streak` などを持たない古い `data.yml` もそのまま読めます。無い項目は0として扱い、
次の受け取りから数え始めます。読み込み時に警告は出ません。
